...
async function startReactNativeServerAsync(projectRoot, options = {}, verbose = true) {
  ...
  let customLogReporterPath;
  const possibleLogReporterPath = (0, _config().projectHasModule)('expo/tools/LogReporter', projectRoot, exp);

  if (possibleLogReporterPath) {
    customLogReporterPath = possibleLogReporterPath;
  } else {
    // TODO: Bacon: Prompt to install expo?
    _Logger().default.global.warn(`Expo is not installed: Using default reporter to format logs.`);
  }
...
async function startExpoServerAsync(projectRoot) {
  ...
  const manifestHandler = async (req, res) => {
    try {
      ...
      let {
        exp: manifest
      } = await (0, _config().readConfigJsonAsync)(projectRoot); // Get packager opts and then copy into bundleUrlPackagerOpts
...
async function startAsync(projectRoot, options = {}, verbose = true) {
  ...
  let {
    exp
  } = await (0, _config().readConfigJsonAsync)(projectRoot, options.webOnly);
