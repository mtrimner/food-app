...
async function determineEntryPointAsync(projectRoot) {
  let {
    exp,
    pkg
  } = await ConfigUtils().readConfigJsonAsync(projectRoot); // entryPoint is relative to the packager root and main is relative
  ...
  const userDefinedEntryPointExists = await _fsExtra().default.pathExists(_path().default.resolve(projectRoot, entryPoint));

  if (!userDefinedEntryPointExists) {
    entryPoint = ConfigUtils().resolveModule('expo/AppEntry', projectRoot, exp);
    const expoEntryPointExists = await _fsExtra().default.pathExists(entryPoint); // Remove project root from file path

    entryPoint = _path().default.relative(projectRoot, entryPoint); // Final existence check
